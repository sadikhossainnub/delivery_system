...
frappe.query_reports["Delivery Charge vs Revenue"] = {
	filters: [
		{
			fieldname: "from_date",
			label: __("From Date"),
	...
	formatter(value, row, column, data, default_formatter) {
		...
		if (column.fieldname === "delivery_status" && data && data.delivery_status) {
			const status = data.delivery_status;
			let color = "grey";
			if (status === "delivered") color = "green";
			else if (status === "cancelled") color = "red";
			else if (["pending", "in_review"].includes(status)) color = "orange";
			else if (status.includes("approval_pending")) color = "blue";
